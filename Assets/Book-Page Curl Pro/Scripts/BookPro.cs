...
public class BookPro : MonoBehaviour
{
    ...
    [HideInInspector]
    public int currentLeftPage = 0;
    [HideInInspector]
    public int currentRightPage = 0;
    ...
    public void UpdatePages()
    {
        ...
        if (previousPaper >= 0) {
            Debug.Log("Left Page " + papers[previousPaper].Back);
            string pageNumber = papers[previousPaper].Back.name.Replace("Page","");
            currentLeftPage = int.Parse(pageNumber);
        }

        if (currentPaper <= papers.Length - 1) {
            Debug.Log("Right Page " + papers[currentPaper].Front);
            string pageNumber = papers[currentPaper].Front.name.Replace("Page","");
            currentRightPage = int.Parse(pageNumber);
        }
