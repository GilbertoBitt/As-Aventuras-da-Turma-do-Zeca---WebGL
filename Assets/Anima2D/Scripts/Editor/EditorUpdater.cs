...
namespace Anima2D 
{
	...
	public class EditorUpdater
	{
		...
		public static void Update(string undoName, bool record)
		{
			...
			{
				if (!control || !control.isActiveAndEnabled || !control.bone) continue;
		...
		{
			if (s_InAnimationMode == AnimationMode.InAnimationMode()) return;
			SetDirty();
		...
		static void OnSceneGUI(SceneView sceneview)
		{
			...
			if (s_LastNearestControl == HandleUtility.nearestControl) return;
		...
		static void UpdateIKs()
		{
			...
			if(Math.Abs(s_LastUpdate - Time.realtimeSinceStartup) < 0.01f)
			{
				return;
			}
